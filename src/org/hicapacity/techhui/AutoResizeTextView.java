...
public class AutoResizeTextView extends TextView {
  ...
  public void resizeText(int width, int height) {
    ...
    // If there is a max text size set, use the lesser of that and the default
    // text size
    ...
    // Until we either fit within our text view or we had reached our min text
    // size, incrementally try smaller sizes
    while (textHeight > height && targetTextSize > mMinTextSize) {
      targetTextSize = Math.max(targetTextSize - 2, mMinTextSize);
      textHeight = getTextHeight(text, textPaint, width, targetTextSize);
    }

    // If we had reached our minimum text size and still don't fit, append an
    // ellipsis
